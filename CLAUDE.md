...
| Scheduler Isolation | 执行域隔离，显式切换(Task/NVMe/Net/RPC) |
| Lock-Free | 框架和应用均无锁 |
...
// RPC 服务端（接受 session 并处理请求）
rpc::serv<service::type::sub, service::type::add>(net_scheduler, session_id)
    >> submit(ctx);

// RPC 客户端（发起调用，支持并发 pipelining）
rpc::call<service::type::add>(net_scheduler, session_id, 10, 20)
...
| `rpc::serv<ids...>(sche, sid)` | RPC服务端：接收请求+分派+响应 | `env/scheduler/rpc/rpc.hh` |
| `rpc::call<id>(sche, sid, args...)` | RPC客户端：发送请求+等待响应 | `env/scheduler/rpc/rpc.hh` |
