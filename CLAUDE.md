...
6. **reduce不因异常终止流** → `reduce()`不会因为某个流元素的异常而停止后续元素的产生。流的终止权在生产者（`for_each`的协程/range）。在流内用`any_exception`捕获异常返回正常值 → reduce继续下一个元素 → 如果下一个元素也异常 → **无限循环**。正确做法：`any_exception`在reduce前捕获异常 → 标记状态（如`cs.close()`） → `just_exception(e)`重新抛出 → 协程下次迭代检查状态返回false → 流自然结束。参考`rpc::server::serv_proc`
