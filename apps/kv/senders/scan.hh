...
#ifndef APPS_KV_SCAN_HH
#define APPS_KV_SCAN_HH
...
namespace apps::kv{
    ...
    inline auto
    scan() {
        return pump::sender::for_each(runtime::index_schedulers.list)
            ...
            >> pump::sender::then([](data::batch *batch, data::b_tree *btree) {
            ...
            >> pump::sender::reduce()
            >> pump::sender::get_context<data::batch *>()
            >> pump::sender::then([](data::batch *batch, ...){
    ...
    start_scan(const char* k) {
        return pump::sender::get_context<data::batch*>()
            >> pump::sender::then([k](data::batch* b, ...){
    ...
    start_scan() {
        return pump::sender::get_context<data::batch*>()
            >> pump::sender::then([](data::batch* b, std::string&& s){
    ...
    next() {
        return pump::sender::get_context<data::batch*>()
            ...
            >> pump::sender::visit()
            >> pump::sender::then([](auto&& arg){
                if constexpr (std::is_same_v<std::true_type, __typ__(arg)>)
                    return pump::sender::just() >> scan();
                else
