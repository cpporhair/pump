...
#ifndef APPS_KV_BATCH_SCHEDULER_HH
#define APPS_KV_BATCH_SCHEDULER_HH
                ...
                scheduler->schedule(
                    new publish_req(
                        ...
                        [context = context, scope = scope]()mutable {
                            pump::core::op_pusher<pos + 1, scope_t>::push_value(context, scope);
                        }
                    )
                );
            }
...
namespace pump::core {
    ...
    struct
    compute_sender_type<context_t, apps::kv::batch::_request_read::sender<scheduler_t>> {
    ...
    template <typename context_t, typename scheduler_t>
    struct
    compute_sender_type<context_t, apps::kv::batch::_publish::sender<scheduler_t>> {
        consteval static auto
        count_value() {
            return 0;
        }
    ...
    template <typename context_t, typename scheduler_t>
    struct
    compute_sender_type<context_t, apps::kv::batch::_allocate_put_id::sender<scheduler_t>> {
        consteval static auto
        count_value() {
            return 0;
        }
...
#endif //APPS_KV_BATCH_SCHEDULER_HH
