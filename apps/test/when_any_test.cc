...
// ============================================================================
// Single-branch async test (regression for dangling pointer fix)
// ============================================================================

void test_when_any_single_async() {
    TEST("when_any single async branch");
    ...
    just()
        >> when_any(
            random_scheduler()->as_task() >> then([]() { return 99; })
        )
        >> then([&](uint32_t winner_index, auto result) {
    ...
    if (completed.load() && got_index.load() == 0 && got_value.load() == 99) {
        PASS();
    } else {
        FAIL("single_async: completed=" + std::to_string(completed.load())
...
// ============================================================================
// Winner index consistency test
// ============================================================================

void test_when_any_winner_consistency() {
    TEST("multi-core: when_any winner index matches value");
    ...
    for (int iter = 0; iter < ITERATIONS; ++iter) {
        ...
        just()
            >> when_any(
                random_scheduler()->as_task() >> then([]() { return 100; }),
                random_scheduler()->as_task() >> then([]() { return 200; }),
                random_scheduler()->as_task() >> then([]() { return 300; })
            )
            >> then([&](uint32_t winner_index, auto result) {
                if (result.index() == 2) {
                    int val = std::get<2>(result);
                    bool consistent = (winner_index == 0 && val == 100) ||
                                      (winner_index == 1 && val == 200) ||
                                      (winner_index == 2 && val == 300);
                    if (consistent) {
    ...
    if (completion_count.load() == ITERATIONS && success_count.load() == ITERATIONS) {
        ...
        FAIL("winner_consistency: completed=" + std::to_string(completion_count.load())
             + "/" + std::to_string(ITERATIONS)
             + " consistent=" + std::to_string(success_count.load()));
    }
}
...
int main() {
    ...
    test_when_any_single_async();
    test_when_any_winner_consistency();
