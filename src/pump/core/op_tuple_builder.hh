#ifndef PUMP_CORE_OP_TUPLE_BUILDER_HH
#define PUMP_CORE_OP_TUPLE_BUILDER_HH

#include "./meta.hh"
#include "./scope.hh"
#include "./concurrent_copy.hh"
...
namespace pump::core {
    ...
    struct reduce_carrier : public op_t {
        ...
        reduce_carrier(op_t&& op) : op_t(__fwd__(op)) { }

        auto concurrent_copy() const {
            return reduce_carrier(core::concurrent_copy(static_cast<const op_t&>(*this)));
        }
    };
    ...
    namespace builder {
        ...
        __ncp__(stream_starter_builder) {
            ...
            push_reduce_op(reduce_op_t&& op) {
                auto builder = push_back(__fwd__(op));
                return builder.parent_builder.push_back(builder.to_op());
            }
...
#endif //PUMP_CORE_OP_TUPLE_BUILDER_HH
