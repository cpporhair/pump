#ifndef PUMP_CORE_META_HH
#define PUMP_CORE_META_HH
...
#define __must_rval__(x) static_assert(::pump::core::must_rvalue<decltype(x)>::v)
#define __all_must_rval__(x...) static_assert(::pump::core::must_rvalue<decltype(x)...>::v)
#define __out_type_name__(x) typedef typename x::out_put_type_name out_put_type_name_##x

#define __forward_values__(x) \
if constexpr (sizeof...(x) > 1) return ::pump::core::tuple_values(__mov__(x)...);\
...
template <typename T>
using underlying_t = std::remove_cv_t<
...
namespace pump::core {
    ...
    struct construct_from_tuple;

    template <typename T, typename... Args>
    struct construct_from_tuple<T, std::tuple<Args...>> {
        static T construct(std::tuple<Args...> &&args) {
            return std::apply(
                []<typename... T0>(T0 &&... args) {
    ...
    template<typename T>
    using remove_ref_if_pointer_t = std::conditional_t<
    ...
    using first_elm_of = std::tuple_element_t<0,std::tuple<env_t...>>;
