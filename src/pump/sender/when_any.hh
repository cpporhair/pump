...
namespace pump::sender {
    ...
    namespace _when_any {
        ...
        // ================================================================
        // single_race_wrapper: zero-overhead path for single branch
        // ================================================================
        template <typename result_variant_t, typename parent_pusher_status_t>
        struct
        __ncp__(single_race_wrapper) {
            result_variant_t result;
            parent_pusher_status_t parent_pusher_status;

            single_race_wrapper(parent_pusher_status_t&& p)
                : parent_pusher_status(__fwd__(p)) {
            }

            single_race_wrapper(single_race_wrapper&&) = delete;
        ...
        __ncp__(starter) {
            ...
            start(context_t& context, scope_t& scope, parent_pusher_status_t&& s) {
                if constexpr (sizeof...(op_list_t) == 1) {
                    // Single branch: zero heap allocation, zero atomic ops
                    using single_wrapper_t = single_race_wrapper<result_t, parent_pusher_status_t>;
                    auto single_wrapper = single_wrapper_t(__fwd__(s));

                    auto new_scope = make_runtime_scope<core::runtime_scope_type::other>(
                        scope,
                        push_back_to_op_list(
                            core::tuple_to_tie(std::get<0>(all_inner_op_list)),
                            reducer<0, single_wrapper_t>(&single_wrapper)
                        )
                    );

                    core::op_pusher<0, __typ__(new_scope)>::push_value(context, new_scope);
                } else {
                    // Multi-branch: heap-allocated race_wrapper with ref counting
        ...
        __ncp__(sender) {
            ...
            auto
            connect() {
                // All branches share the same result variant type
                using first_sender_t = std::tuple_element_t<0, std::tuple<sender_t...>>;
                using result_t = typename when_any_res<
                    typename decltype(when_any_compute<context_t, first_sender_t>::get_value_type_identity())::type
                >::type;
