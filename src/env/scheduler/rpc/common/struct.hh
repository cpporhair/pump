...
namespace pump::scheduler::rpc::server {
    ...
    rpc_frame_helper {
        ...
        realloc_frame(uint32_t new_size) {
            if (frame == nullptr) {
                frame = static_cast<rpc_frame *>(malloc(new_size + sizeof(rpc_header)));
            }
            else if (frame->header.total_len < new_size + sizeof(rpc_header)) {
                frame = static_cast<rpc_frame *>(realloc(frame, new_size + sizeof(rpc_header)));
            }
            std::memcpy(&frame->header, &frame->header, sizeof(rpc_header));
            frame->header.total_len = new_size + sizeof(rpc_header);
        }
    };
